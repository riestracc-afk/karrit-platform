...
if (Test-Path $logoSilver) {
    ...
    $bmp.Dispose()
    Write-Output "SILVER: Total=$total, Alpha=$alpha, BBox=[$minX,$minY] to [$maxX,$maxY]"
} else { Write-Output "SILVER: Not Found" }

if (Test-Path $logoBCO) {
    $bmp = New-Object System.Drawing.Bitmap($logoBCO)
    $c1 = $bmp.GetPixel(0,0)
    $c2 = $bmp.GetPixel(0, $bmp.Height-1)
    $c3 = $bmp.GetPixel([int]($bmp.Width/4), [int]($bmp.Height/2))
    $bmp.Dispose()
    Write-Output "BCO: TL=($($c1.A),$($c1.R),$($c1.G),$($c1.B)), BL=($($c2.A),$($c2.R),$($c2.G),$($c2.B)), CL=($($c3.A),$($c3.R),$($c3.G),$($c3.B))"
} else { Write-Output "BCO: Not Found" }
